...
impl StorageHandle {
  ...
  pub async fn generate_public_key(&self, type_: KeyType, location: KeyLocation<'_>) -> Result<PublicKey> {
    self.data.lock().await.generate_public_key(type_, location).await
  }
